# L1 KB: Missing Desktop Shortcuts - Quick Fix (FLR6-PROF-003)

**Article ID:** FLR6-PROF-003-L1  
**Audience:** Floor 6 Users  
**Version:** 1.1  
**Date:** 14-Aug-2026

Source: Re-expressed from [Runbook-FLR6-PROF-003-Missing-Shortcuts.md](Runbook-FLR6-PROF-003-Missing-Shortcuts.md).

If your desktop shortcuts disappeared, your files are still safe. Shortcuts are only links.

Try this:
1. Open Start, search the app name (for example Outlook).
2. Right-click the app.
3. Select Send to -> Desktop (create shortcut).
4. Repeat for each app you use.

If a shortcut still does not appear:
1. Restart once.
2. Check Recycle Bin for .lnk files and restore them.
3. Contact IT if needed.

Contact IT Help Desk:
- Ext 555-1234
- servicedesk@company.com

Status: Low-severity usability issue; recovery is available.
